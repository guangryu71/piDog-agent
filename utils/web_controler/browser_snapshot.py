...
def build_role_snapshot_from_aria(
    ...
    *,
    ...
    Returns (snapshot_text, refs_dict) where refs_dict maps ref->{role,name,nth}.
    """
    ...
    if options.get("interactive"):
        ...
        for line in lines:
            depth = _get_indent_level(line)
            if options.get("maxDepth") is not None and depth > options["maxDepth"]:
                continue
            m = re.match(r'^(\s*-\s*)(\w+)(?:\s+"([^"]*)")?(.*)$', line)
            if not m:
                continue
