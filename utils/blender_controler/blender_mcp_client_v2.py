            ...
            try:
                ...
                if response_str:
                    ...
                    if response.get("executed") == True or response.get("status") == "success":
                        ...
                        # 没有明确成功标志，尝试从响应中提取有用信息
                        raw_obj = response_str[:500]
                        _log(f"Blender responded but without executed/status flag. Raw: {raw_obj}", "WARN")

                        # 尝试提取 JSON-RPC result（如果是 MCP 格式响应）
                        possible_result = (
                            response.get("result") or
                            response.get("data") or
                            response.get("output") or
                        ...
                        if possible_result:
                            if isinstance(possible_result, dict):
                                possible_result = json.dumps(possible_result, indent=2, ensure_ascii=False)
                            elif isinstance(possible_result, list):
                                possible_result = json.dumps(possible_result, indent=2, ensure_ascii=False)
                            return {
                                "status": "success",
                                "result": str(possible_result),
                                "message": f"代码已发送到Blender，收到响应（格式{i+1}）",
                                "format_used": i + 1,
                                "note": "Blender acknowledged the request. The code was sent and processed."
                            }

                        # 完全无法解析响应内容，但连接成功 → 代码已送达
                        return {
                            "status": "success",
                            "result": raw_obj,
                            "message": f"代码已发送到Blender（格式{i+1}，收到响应但未识别状态字段）",
                            "format_used": i + 1,
                            "note": "Code was transmitted successfully. Blender acknowledged receipt."
                        }
                else:
